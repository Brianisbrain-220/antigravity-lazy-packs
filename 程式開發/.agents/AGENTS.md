...
- 專案要引年的圖片請放在專案 `assets/` 或 Obsidian 附件資料夾。
...
### 跨工具交接 (與 Claude 協作)
使用者說「交接」、「換 Claude」或「收工交接」時，AI 應遵循 `cross-tool-handoff-guardian` 規範：
1. **嚴禁濫建交接檔**：嚴禁在專案目錄建立 `README_HANDOVER.md`、`TODO.md` 或交接日誌。
2. **唯一事實來源 (SSOT)**：所有架構演進與決議一律寫入 Obsidian 專案駕駛艙（`Bound_Project_*.md`）。
3. **強制決議區塊**：涉及重大架構決策時，必須套用 `> **決議(YYYY-MM-DD)**:` 標準格式。
4. **雲端推播確認**：若為 GAS 專案，必須確認已執行 `clasp push` 並提醒部署新版本。
5. **產出對等指令**：回報時附帶結構化的交接 Prompt 供傳遞給 Claude。
