...
description: Clasp 雲端部署 + Netlify MCP 零複製雙向閉環網頁系統。說「連接 clasp netlify」「部署 clasp netlify」時載入。
---
...
* **應變對策**：引導使用者手動點開專案的線上編輯器，在上方選單選擇任意函數（如 `getSheet`），點擊 **「執行」**，並完成 **「審閱權限」->「选择帳號」->「進階」->「前往專案（不安全）」->「允許」** 的 Google 授權確認。
